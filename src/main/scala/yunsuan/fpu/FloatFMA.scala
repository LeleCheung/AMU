...
// shift function
  ...
  // shift srcValue left by shiftValue
  ...
  // shift srcValue right by shiftValue, with sticky bit
...
//*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // fire valid signal: 3 stage pipeline
  val fire = io.fire
  val fire_reg0 = GatedValidRegNext(fire)
  val fire_reg1 = GatedValidRegNext(fire_reg0)

  // op code
  val is_fmul   = io.op_code === FmaOpCode.fmul // fp multiply
  val is_fmacc  = io.op_code === FmaOpCode.fmacc // fp multiply and add
  val is_fnmacc = io.op_code === FmaOpCode.fnmacc // fp multiply and add neg
  val is_fmsac  = io.op_code === FmaOpCode.fmsac // fp multiply and subtract
  val is_fnmsac = io.op_code === FmaOpCode.fnmsac // fp multiply and subtract neg
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  val RNE = io.round_mode === "b000".U // round to nearest
  val RTZ = io.round_mode === "b001".U // round towards zero
  val RDN = io.round_mode === "b010".U // round down
  val RUP = io.round_mode === "b011".U // round up
  val RMM = io.round_mode === "b100".U // round to nearest with ties to max magnitude
...
private[fpu] class BoothEncoderF64F32F16(width : Int = 53, is_addend_expand_1bit : Boolean = true) extends Module{
  ...
  val outNum  = width/2 + 1 // the number of partial products
  ...
  val io = IO(new Bundle() {
    ...
    val out_pp = Output(Vec(outNum,UInt(addend_seq_width.W))) // outNum partial products
  })
...
//*///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Carry Save Adder: N to 2 (using several CSA4to2 and CSA3to2)
private[fpu] class CSA_Nto2With3to2MainPipeline(n :Int, width :Int, pipeLevel :Int) extends Module{
  val io = IO(new Bundle() {
    val fire    = Input(Bool())
    val in      = Input(Vec(n,UInt(width.W))) // n input
  ...
  // calculate the number of CSA4to2 and CSA3to2
